...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    async function loadMe() {
      try {
        const me = await apiFetch<{ user: any }>("/auth/me", { method: "GET", token: token ?? undefined });
  ...
  useEffect(() => {
    ...
    async function loadCandidates() {
      const res = await apiFetch<{ candidates: any[] }>("/public/candidates", { method: "GET" });
      setCandidates((res.candidates || []).map((x) => ({ id: x.id, name: x.name, party: x.party })));
    }
    void loadCandidates();
  }, [token]);
  ...
  async function onStartElection() {
    ...
    try {
      ...
      await apiFetch("/admin/election/start", {
        method: "POST",
        token,
  ...
  async function onCreateCandidate() {
    ...
    try {
      await apiFetch("/admin/candidates", {
        method: "POST",
        token,
        body: { name: candName, party: candParty },
      });
      setCandName("");
      setCandParty("");
